...
> 神经网络不能直接处理<span style="color:red">标量</span>数据，核心设计是面向<span style="color:red">高维向量</span>输入进行处理与计算的。

- 特征构造：补充相关衍生特征。（温度：结合时间维度（早/中/晚）、天气状况（晴/雨/雪）等）
- 嵌入映射：通过嵌入层将标量映射到低维高维向量空间。（时间：结合三角函数）
- 数据扩展：可收集相关的数据。（用户单次消费金额：结合用户的消费频率、历史消费总额等）
...
| :----: | :------------------: |
| 输入层 | 输入:  $x_1,x_2,x_3$ |
| 隐藏层 | 输入：$\delta_4, \delta_5, \delta_6$<br />激活值：$\alpha_4, \alpha_5, \alpha_6$ |
| 输出层 | 输入：$\delta_7, \delta_8, \delta_9$<br />预测值：$\hat{y}_7, \hat{y}_8, \hat{y}_9$ |
...
![神经网络-1](../Attachments/Deep Learning/神经网络-1.png)
